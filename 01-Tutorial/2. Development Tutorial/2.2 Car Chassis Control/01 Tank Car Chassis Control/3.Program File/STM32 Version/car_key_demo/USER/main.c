...
int8_t p0[4]={0,0};      										 //停止(stop)
int8_t p1[4]={-50,-50};      						  //前进(forward)
int8_t p2[4]={50,50};      							//后退(backward)
int8_t p3[4]={50,-50};      							//左转(turn left)	                      
int8_t p4[4]={-50,50};    								//右转(turn right)
...
int main(void)
{
  ...
  while(1)
	...
	{
		case 0:
		I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p0,2);	
    DelayMs(1800);			
		break;
		
		case 1:           //前进(forward)
		I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p1,2);						
		DelayMs(1800);
		break;
		
		case 2:        //后退(backward)
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p2,2);						
	  DelayMs(1800);			
		break;
		
		case 3:     //左转(turn left)
    I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p3,2);						
	  DelayMs(1800);					
		break;
		
	  case 4:   //右转(turn right)
    I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p4,22);						
	  DelayMs(1800);				
		break;
