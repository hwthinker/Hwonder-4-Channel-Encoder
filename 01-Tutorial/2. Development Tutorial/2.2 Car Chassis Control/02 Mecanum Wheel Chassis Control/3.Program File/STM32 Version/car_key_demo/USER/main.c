...
int8_t I2C_Write_Len(int8_t Reg,int8_t *Buf,int8_t Len)//I2C的写数据(I2C write data)
{
	uint8_t i;
...
int8_t p1[4]={-50,50,50,-50};      						  //前进(forward)
int8_t p2[4]={50,-50,-50,50};      							//后退(backward)
int8_t p3[4]={50,50,50,50};      						    //左移(move to the left)
int8_t p4[4]={-50,-50,-50,-50};      					  //右移(move to the right)
int8_t p5[4]={50,-50,50,-50};      							//右转(turn right)                      
int8_t p6[4]={-50,50,-50,50};    								//左转(turn left)
int8_t p7[4]={0,50,0,50};      									//左漂移(drift left)
int8_t p8[4]={0,50,50,0};      									//左前(left front)         
int8_t p9[4]={-50,0,0,-50};    								  //右前(right front) 
int8_t p10[4]={0,-50,-50,0};      							//右后(right rear)                       
int8_t p11[4]={50,0,0,50};    									//左后(left rear)
...
int main(void)
{
  ...
  while(1)
		...
		switch(key_num)
	{
		...
		case 1:           //前进后退(forward and backward)
		...
		case 2:        //转向(turn)
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p5,4);						
	  DelayMs(1800);
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p6,4);						
	  DelayMs(1800);
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p0,4);					
		break;
		
		case 3:     //前后左右(forward, backward, left, and right)
    I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p1,4);						
	  DelayMs(1800);
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p2,4);				
	  DelayMs(1800);
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p3,4);						
	  DelayMs(1800);
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p4,4);						
	  DelayMs(1800);
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p0,4);						
		break;
		
	  case 4:   //斜向运动(move diagonally)
    I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p8,4);						
	  DelayMs(1800);
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p9,4);						
	  DelayMs(1800);
  	I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p10,4);						
  	DelayMs(1800);
  	I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p11,4);						
  	DelayMs(1800);	
	  I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p0,4);						
		break;
		
    case 5:	//漂移(drift)
    I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p7,4);						
	  DelayMs(1800);
    I2C_Write_Len(MOTOR_FIXED_SPEED_ADDR,p0,4);						
    break;		
		default:
